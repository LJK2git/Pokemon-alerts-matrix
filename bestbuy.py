...
# Overridable via the DEBUG_DIR env var (the Dockerfile sets this to an
# absolute path, /app/debug_runs). That directory sits inside the project
# folder that docker-compose bind-mounts straight through to the host
# (`./:/app`), so screenshots always land at code/matrix/debug_runs on your
# actual machine -- pinning this to an absolute path removes any ambiguity
# from what the process's cwd happens to be when it starts.
DEBUG_DIR = Path(os.environ.get("DEBUG_DIR", "debug_runs"))
DEBUG_DIR.mkdir(exist_ok=True, parents=True)
print(f"[i] Debug artifacts will be saved to: {DEBUG_DIR.resolve()}")

# Set IN_DOCKER=1 in the container (the Dockerfile does this) so Chrome gets
# the flags it needs to run as root inside a container: Chrome refuses to
# use its sandbox as root, and --disable-dev-shm-usage avoids /dev/shm
# running out even with shm_size bumped in docker-compose.yml.
...
def check_for_invite(url: str, headless: bool = True, debug_tag: str = "run") -> bool:
    ...
    if IN_DOCKER:
        # Chrome refuses to run as root with its sandbox enabled, and this
        # container doesn't create a non-root user -- without this flag,
        # SB(...) fails to launch before check_for_invite ever gets far
        # enough to save a screenshot, which is why debug_runs could stay
        # empty even though a "check" technically ran.
        sb_kwargs["chromium_arg"] = "--no-sandbox,--disable-dev-shm-usage,--disable-http2"
...
def check_urls_for_invite(urls, headless: bool = True):
    ...
    for i, url in enumerate(urls):
        ...
        try:
            ...
            # Full traceback so a browser-launch failure (e.g. Chrome not
            # installed/misconfigured in Docker) is actually diagnosable
            # from `docker compose logs`, instead of a single swallowed line.
            traceback.print_exc()
            found = False
